...
AI가 짠 UI, 어딘가 촌스러운데 정확히 뭐가 문제인지 짚어내기 어려울 때가 있다. 반사적 장식(오버라인·글로우·그라디언트 오브), 규율 없는 그리드, 과밀한 간격, 장식 이탤릭, 무지개 팔레트 — 흔한 슬롭 신호는 대개 다섯 영역으로 좁혀진다. slopslap 은 문답 없이 병렬 점검 파이프라인으로 그 신호를 걷어낸다.

## The Areas

![inspection areas](docs/assets/readme/inspection-areas.png)

- **A · 대표 슬롭 기계 제거** — 측정 없이 "보이면 삭제": 오버라인·eyebrow, 플로팅 오브·blob, mesh/글로우, 글래스모피즘, 소프트섀도 남발, 그라디언트 텍스트, 이모지 아이콘, 장식 도트·스파클
