# 배포 계획 — slop-quick (Claude Code plugin)

`slop-quick` 스킬을 Claude Code **plugin**으로 패키징해 git + marketplace 로 배포하는 계획.
...
현재 7개 파일이 slop-quick 의 최소 의존 세트다(별도 정리 불필요):

```
.claude/skills/slop-quick/SKILL.md              # 스킬 진입점
.claude/skills/slop-quick/references/inspection-areas.md  # 규칙 SSOT + check 스키마
...
sed -i '' 's#scripts/scan-slop-signals.mjs#${CLAUDE_PLUGIN_ROOT}/scripts/scan-slop-signals.mjs#g; s#scripts/build-findings-report.mjs#${CLAUDE_PLUGIN_ROOT}/scripts/build-findings-report.mjs#g; s#src/data/aiSlopTaxonomyData.js#${CLAUDE_PLUGIN_ROOT}/src/data/aiSlopTaxonomyData.js#g' \
  .claude/skills/slop-quick/SKILL.md .claude/skills/slop-quick/references/inspection-areas.md
```
...
```json
{
  ...
  "plugins": [
    { "name": "slop-quick", "source": ".", "description": "AI-slop 병렬-점검 파이프라인 스킬" }
  ]
}
```
...
# (1번 sed 실행)
git add -A
git commit -m "release: slop-quick v1.0.0 (plugin 패키징 + 경로 ${CLAUDE_PLUGIN_ROOT} 전환)"
git tag v1.0.0

# GitHub 원격 연결 + 푸시
git remote add origin https://github.com/groovelb/slop-quick.git   # 레포명 예시
...
```bash
/plugin marketplace add groovelb/slop-quick      # GitHub owner/repo
/plugin install slop-quick@groovelb-slop
/slop-quick                                       # 또는 namespace: /slop-quick:slop-quick
```
...
# 사용자: /plugin update slop-quick@groovelb-slop
```
...
- [ ] `/plugin marketplace add` → `/plugin install` → `/slop-quick` E2E 확인
