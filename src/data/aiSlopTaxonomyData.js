        ...
        groups: [
          {
            label: null,
            items: [
              { id: 'styling-before-content-model', detect: { kind: 'judgment', note: '역산 검사(audit 0단계): 화면의 콘텐츠 인벤토리(무슨 텍스트·이미지·데이터를 보여주는가)를 우선순위 목차로 재구성했을 때 레이아웃 칸의 크기·순서가 그 우선순위의 함수인지 확인. 아니면 스타일이 콘텐츠 모델보다 먼저 결정된 것. 콘텐츠 목차화·상수화(slop-quick 0단계)가 양화 절차다. 이 검사를 통과하지 못한 화면에 "AI 티 없음" 판정을 내릴 수 없다.' }, name: 'Styling Before Content Model', koName: '콘텐츠 모델 없는 스타일 선행', description: '콘텐츠를 목차화·상수화해 우선순위를 정하기 전에 스타일 무드부터 입힌다. 레이아웃이 콘텐츠의 함수가 아니라 무드의 함수가 된다.', tell: '표면 스타일은 화려한데 어떤 칸이 왜 그 크기인지 설명이 안 된다. 규율 없는 그리드·질식 밀도·수제 연출 알리바이 등 2세대 슬롭 전반의 상류 원인.', whyDisliked: '디자인의 첫 결정(무엇을 어떤 순서로 보여줄 것인가)이 빠져 있어, 스타일을 걷어내면 남는 것이 없다.', severity: 'strong', cause: 'underspec', aliases: ['mood-first design', '기획 없는 스타일'], escapeNote: '콘텐츠를 먼저 목차화·상수화하고(slop-quick 0단계 콘텐츠 상수화), 전역 그리드 모듈을 정의한 뒤, 레이아웃 칸을 우선순위의 함수로 재설계한다. 스타일은 마지막이다.', source: '사용자 관찰 2026-07 (audit 1순위 검증 항목)' },
